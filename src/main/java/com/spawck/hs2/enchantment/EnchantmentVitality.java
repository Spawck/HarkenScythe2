 ...
 * (http://bit.ly/18EyAZo)
 *
 * File created @[Mar 5, 2015, 10:41:55 AM]
 */
...
{
	public EnchantmentVitality(int effectId)
	{
		super(effectId, 1, EnumEnchantmentType.armor);
		this.setName(Names.Enchantments.VITALITY);
	}
	...
	public static int getTotalEnchantmentLvl(EntityPlayer player)
	{
		...
		for (int armorInvSize = 0; armorInvSize < 4; armorInvSize++)
		{
			if (player.inventory.armorItemInSlot(armorInvSize) != null)
			{
				...
				enchantmentBonus += EnchantmentHelper.getEnchantmentLevel(HSEnchantments.vitality.effectId, armor);
			}
		}
